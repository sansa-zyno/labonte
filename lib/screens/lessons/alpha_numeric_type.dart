...
class _AlphaNumericTypeState extends State<AlphaNumericType> {
  ...
  void initState() {
    ...
    textToSpeechProvider.playFullAudio(result: newMap, lessonIndex: widget.lessonData.lessonIndex, snapshot: widget.snapshot, filename: filename);
  }
                ...
                callBack: () async {
                  if (textToSpeechProvider.playerState == AudioPlayerState.playing) {
                    ...
                    await textToSpeechProvider.playFullAudio(
                        result: newMap, lessonIndex: widget.lessonData.lessonIndex, snapshot: widget.snapshot, filename: filename);
                  }
                },
              ),
                    ...
                    children: newMap.entries
                        .map((item) => InkWell(
                              onTap: () {
                                ...
                                textToSpeechProvider.playPronunciation(item.key.toString());
                              },
                              child: Column(
